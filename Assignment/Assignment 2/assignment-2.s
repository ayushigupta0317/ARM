...
3.)  In CODE-1, Then instructions and Else instruction is written without any condition, i.e. no condition used in ITTTE block.

4.)  In CODE-2, then block use LT conditional instructions whereas else block uses GE conditional instruction. 

5.)  As per rule of if-then-else, else condition must be inverse of then condition, here LT inverse of GE.
